...
export const classicGridConverter: GridConverter = (grid) => {
  ...
  grid.iterate((cell, pos2d) => {
    if (cell === null) {
      ...
      for(let i = 1; i <= corridorLength; i++) {
        ...
        physicalGrid.getSliceByLogicalPosition(corridorLeft).set(VerticalLayer.Down1, stairWallLeft(stairDir))
        physicalGrid.getSliceByLogicalPosition(corridorRight).set(VerticalLayer.Down1, stairWallRight(stairDir))
      }
    }
  })
