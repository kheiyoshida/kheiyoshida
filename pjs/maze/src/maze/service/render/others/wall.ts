import p5 from 'p5'
import { colorCopy, pushPop, randomColor } from 'p5utils/src/p5utils'
import { random } from "p5utils/src/random"
import { randomBetween } from "p5utils/src/random"
import { Conf } from '../../../config'
import { getDeadEndItem, putDeadEndItem } from '../../../domain/maze/deadend'
import { sequence, DrawFn, moment } from 'p5utils/src/sequence'
import { getPaletteColor } from 'src/maze/domain/vision/color/palette'

export const emptyPicture = (pos: number[], size: number[]) => {
  pushPop(() => {
    p.fill(getPaletteColor('wallPicture'))
...
const setupPicture = (size: number[]) => {
  ...
  g.background(getPaletteColor('wallPicture'))
  const c = colorCopy(getPaletteColor('wallPicture'))
  c.setAlpha(5)
  g.fill(c)
  g.stroke(255)
  unit = g.height / 30
...
const graphicOverDraw = (g: p5.Graphics) => {
  g.push()
  g.stroke(getPaletteColor('mellowStroke'))
...
const initSeeds = (g: p5.Graphics) => {
  const seedNum = randomBetween(FlowerConf.minSeedNum, FlowerConf.maxSeedNum)
...
const grow = (g: p5.Graphics, root: p5.Vector, n = 0) => {
  ...
  const v = root.copy()
  v.add(p.random(-unit * 2, unit * 2), p.random(-unit * 3, 0))
...
const bloom = (g: p5.Graphics, root: p5.Vector) => {
  ...
  const circle = Array(directions)
    .fill(0)
    .map((_, x) => x * devided + vari)
  const rootDegree = randomBetween(0, 360)
  const petals: DrawFn[] = []
  for (const degree of circle) {
    petals.push(() => petal(g, root, degree, rootDegree))
  }
  return petals
}
...
const petal = (
  ...
  const next: DrawFn[] = []
  if (random(0.8)) {
    next.push(() => petal(g, v, degree, rootDegree - vari, n + 1))
    next.push(() => petal(g, v, degree, rootDegree + vari, n + 1))
  }
  return next
}
