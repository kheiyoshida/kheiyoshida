...
bool detectEdge() {
    ...
    float offset = (1.0) / 691.0; // depth texture resolution = canvas.width
...
const float baseColorDiff = 0.5;

void main() {
    vec3 originalColor = vec3(texture(ColorTexture, TexCoords));

    vec3 lineColor;
    float edgeStrokeColorDiff = mix(0.1, baseColorDiff, uEdgeRenderingLevel);
