import p5 from 'p5'
import { destVect } from 'p5utils/src/render'
...
export class Wing {
  ...
  private wing(origin: p5.Vector) {
    ...
    return Math.floor(randomBetween(0, 12)) == 0 ? [d2, d3] : [d2]
  }
}
...
export function simmer(v1: p5.Vector, v2: p5.Vector) {
  for (let i = 1; i <= DOTS; i += 1) {
    p.point(weighted(v1.x, v2.x, i, DOTS), weighted(v1.y, v2.y, i, DOTS))
  }
}

const weighted = (a: number, b: number, w: number, all: number) => {
