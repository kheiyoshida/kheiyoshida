...
uniform vec2 uGradientTexelSize; // gradient pass size
...
void main() {
    ...
    vec2 offset01 = vec2(float(bestIJ.x)/uTileSize, float(bestIJ.y)/uTileSize);

    fragColor = vec4(best, offset01.x, offset01.y, 1.0);
}
