...
const FlexibleImage = ({path, onClick, margin, link}:
  {
    path: string
    onClick: () => void
    margin: boolean
    ...
    >
      {
        link ? 
          <Link 
            href={link} 
            className={styles.images__item__link} 
          /> : null
      }
      <Image
    ...
    <Modal close={close}>
      <div className={styles.display} onClick={close}>
        ...
        <div className={styles.display__control} onClick={(e) => e.stopPropagation()}>
          <div onClick={prev} style={{visibility: prevOk ? 'visible' : 'hidden'}}>⇦</div>
          <div onClick={close}>●</div>
          <div onClick={next} style={{visibility: nextOk ? 'visible' : 'hidden'}}>⇨</div>
        </div>
      </div>
    </Modal>
  )
